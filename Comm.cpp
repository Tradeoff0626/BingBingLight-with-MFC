...
CComm::CComm( )
{
	...
	osRead.hEvent = NULL;		//예외처리용 코드
	osWrite.hEvent = NULL;		//예외처리용 코드
}
...
DWORD CommWatchProc(LPVOID lpData)
{
	...
	if ( os.hEvent == NULL )
	{
		MessageBox( NULL, L"Failed to create event for thread!", L"comm Error!", MB_ICONEXCLAMATION | MB_OK );
		return ( FALSE ) ;
	}
...
// 컴포트에 데이터를 쓴다.
BOOL CComm::WriteCommBlock( LPBYTE lpByte , DWORD dwBytesToWrite)
{
	DWORD		dwErrorFlags;
	BOOL        fWriteStat ;
